...
begin
  ...
  begin
    Error := GetLastError;
    MessageDlg(Format(sShellExecuteFileOpenError, [Filename, SysErrorMessage(Error)]), mtWarning, [mbOk], 0);
  end;
end;
...
begin
  try
    ...
    begin
      ...
      MessageDlg(Format('Invalid URL: %s'#13'%s', [s, E.Message]), mtWarning, [mbOk], 0);
      exit;
    end;
  end;
  ...
  begin
    Error := GetLastError;
    MessageDlg(Format(sShellExecuteBrowseError, [FinalURL, SysErrorMessage(Error)]), mtWarning, [mbOk], 0);
  end;
end;
